	...
	def _import_clicked(self):
		...
		if file_path:  # if a file was selected
			...
			self._master.switch_frame(
				Loadingframe(self._master, "File loading.", ResultFrame, process_workbook, [workbook]))


def process_workbook(input_workbook: openpyxl.Workbook, caller, target_frame: Frame):
	sheet = input_workbook.active

	intermediate_array = [column for column in
						  sheet.iter_rows(min_row=1, max_col=sheet.max_column, max_row=sheet.max_row, values_only=True)]
